...
#endif
		if (sector) {
		  if(ramdisk_read(fs->drv, fs->win, sector, 1) != RES_OK)
		    return FR_DISK_ERR;
		      fs->winsect = sector;
		}
	}
...
{
	if (ramdisk_read(fs->drv, fs->win, sect, 1) != RES_OK)
	  return 3;
...
)
{
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	/* We removed the original code here, which analyzed the BPB to initialize
	 * the FS object, and instead just use the offset given in the parameters to
	 * f_mount() (and discover the other information ourselves). */

	fs->fs_type = 0;					/* Clear the file system object */
	fs->drv = (BYTE)LD2PD(vol);			/* Bind the logical drive and a physical drive */

/* CN: This is already done by Bear. */
#if 0
...
#if 0 /* This is where we replace their code with our code. */
	...
	if (ramdisk_read(fs->drv, fs->win, fs->fsi_sector, 1) == RES_OK &
...
/* Mount *now* (no deferred mounting like in f_mount), at the given offset in
 * the hard drive (no scanning the MBR like in f_mount/f_chkmount). If the
 * offset is wrong, this will fail in a way I haven't tried to search the code
 * to understand, so make sure the offset is right.
*/
FRESULT f_mount_offset(BYTE vol, FATFS *fs, uint64_t offset) {
	FATFS *rfs;
...
)
{
	...
	//kputs("past chk_mounted");
#endif
	INIT_BUF(dj);
	if (res == FR_OK) {
		res = follow_path(&dj, path);	/* Follow the file path */
		//kputs("follow_path returned");
	}
	dir = dj.dir;
	...
	if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
		...
		if (res == FR_OK) {						/* Follow succeeded */
			if (dir[DIR_Attr] & AM_DIR) {		/* It is a directory */
				//kputs("RES WILL BE SET TO FR_NO_FILE");
				res = FR_NO_FILE;
			} else {
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				if (ramdisk_read(fp->fs->drv, rbuff, sect, (BYTE)cc) != RES_OK)
...
#endif
				if (ramdisk_read(fp->fs->drv, fp->buf, sect, 1) != RES_OK)
                                        ABORT(fp->fs, FR_DISK_ERR);^
			}
#endif
			fp->dsect = sect;
	...
	*bw = 0;	/* Initialize byte counter */
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			...
			if(fp->fptr < fp->fsize && ramdisk_read(fp->fs->drv, fp->buf, sect, 1) != RES_OK
                                ABORT(fp->fs, FR_DISK_ERR)
			}
#endif
			fp->dsect = sect;
...
#endif
					if (ramdisk_read(fp->fs->drv, fp->buf, dsc, 1) != RES_OK)  
                                                ABORT(fp->fs, FR_DISK_ERR)
#endif
					fp->dsect = dsc;
				}
...
#endif
			if(ramdisk_read(fp->fs->drv, fp->buf, nsect, 1) != RES_OK)
                                ABORT(fp->fs, FR_DISK_ERR)
#endif
			fp->dsect = nsect;
		}
...
)
{
	...
	/* Get disk statics */
/* CN: Disk is already initialized */
#if 0
